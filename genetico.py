...
def mutacao(individuo):
    ...
    for i in range (len(individuo)):
        for j in range (len(individuo[0])):
            vaiMutar = np.random.uniform(0, 1, 1)
            if (vaiMutar <= CHANCE_MUT):
...
def crossover(individuo1, individuo2):
    ...
    for i in range (len(filho)):
        for j in range(len(filho[0])):
            temCross = np.random.uniform(0, 1, 1)
            if (temCross <= CHANCE_CO):
...
def proxima_geracao(populacao, fitness):
    ...
    proxima_ger = []
    populacao_ordenada = ordenar_lista(populacao, fitness, decrescente=True)
    populacao_mantida = populacao_ordenada[0:NUM_MELHORES]
    for i in populacao_mantida:
        proxima_ger.append(i)
...
for ger in range(num_geracoes):
    ...
    fitness = []
    for ind_g in populacao:
        fit_ind = calcular_fitness(jogo, ind_g)
